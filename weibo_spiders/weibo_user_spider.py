...
class WeiboUserSpider(object):
    ...
    def user_to_csv(self):
        ...
        if not os.path.isdir('users'):
            os.makedirs('users')
        first_user = self.user_config_list[0]["user_id"]
        file_path = f'users/{first_user}.csv'
        result_headers = [
    ...
    def get_filepath(self, type):
        """获取结果文件路径"""
        try:
            file_dir = 'users' + os.sep + self.user['screen_name']
            # file_dir = os.path.split(
    ...
    def start(self):
        """运行爬虫"""
        try:
            for user_config in tqdm(self.user_config_list, desc='User'):
